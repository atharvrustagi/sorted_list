...
class sorted_list   {
    ...
    int _at_index(Node* root, int &index)   {
        if (root->left && root->left->count > index)
            return _at_index(root->left, index);
        index -= root->left ? root->left->count : 0;
        if (!index)
            return root->val;
        --index;
        return _at_index(root->right, index);
    }

    int _index_of(Node *root, int key)  {
        if (!root)
            return -1;
        if (key < root->val)
            return _index_of(root->left, key);
        else if (key == root->val)
            return (root->left ? root->left->count : 0);
        int ret = _index_of(root->right, key);
        if (ret == -1)
            return -1;
        return (root->left ? root->left->count : 0) + ret + 1;
    }
...
public:
    sorted_list() : root(nullptr), tree_size(0)  {}
    ...
    // Returns a boolean representing the presence of the key in the tree
    bool find(int key)  {
        return _find(key, root);
    }

    // Retrieves the key at the given index in the sorted list, in logarithmic time
    int at(int index)   {
        if (index < 0 || index >= tree_size)
            return 0;
        return _at_index(root, index);
    }
    ...
    // displays the level order traversal of the tree
    void level_order();

    // Returns the index of the key if present, else -1
    int index_of(int key)   {
        return _index_of(root, key);
    }
